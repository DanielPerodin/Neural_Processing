...
def calculate_metrics(ch_data_snippet, fs, preprocessor_params):
    ...
    try:
        ...
        # --- Signal: |mean of per-spike peak amplitudes| and SNR ---
        # Find contiguous runs of samples below threshold, take the minimum
        # (most negative) sample within each run as that spike's peak.
        below = flat < threshold
        peak_values = []
        in_spike = False
        current_min = 0.0
        for i, val in enumerate(flat):
            if below[i]:
                ...
                if in_spike:
                    peak_values.append(current_min)
                    in_spike = False
        # Close an open spike at the end of the snippet
        if in_spike:
            peak_values.append(current_min)

        if peak_values:
            mean_peak_signal = float(abs(np.mean(peak_values)))
            results['rms_signal_uV'] = mean_peak_signal   # field reused for |mean peak|
            if results['rms_noise_uV'] > 0:
                results['snr'] = mean_peak_signal / results['rms_noise_uV']
        else:
...
def main():
    ...
    print("Signal  : |mean of per-spike peak amplitudes| (spikes > 3×MAD)")
    print("SNR     : |mean spike peak| / RMS noise")
    print("=" * 60)
    ...
    for folder in subfolders:
        ...
        try:
            ...
            for ch in included_channels:
                ...
                row[f'{p}_MeanPeak_uV']    = round(m['rms_signal_uV'], 3)
